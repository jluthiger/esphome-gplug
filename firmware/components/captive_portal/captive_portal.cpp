...
void CaptivePortal::handleRequest(AsyncWebServerRequest *req) {
  ...
  // gPlug: served from this->index_gz_/index_gz_len_ (set by set_index(), see __init__.py),
  // not the upstream vendored INDEX_GZ/captive_index.h.
#ifndef USE_ESP8266
  auto *response = req->beginResponse(200, ESPHOME_F("text/html"), this->index_gz_, this->index_gz_len_);
#else
  auto *response = req->beginResponse_P(200, ESPHOME_F("text/html"), this->index_gz_, this->index_gz_len_);
#endif
