...
class CaptivePortal : public AsyncWebHandler, public Component {
 public:
  ...
  // gPlug: runtime page source, set from codegen (see __init__.py); replaces upstream's
  // compile-time INDEX_GZ/captive_index.h.
  void set_index(const uint8_t *data, size_t len) {
    this->index_gz_ = data;
    this->index_gz_len_ = len;
  }
