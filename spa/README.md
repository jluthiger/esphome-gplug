...
| Setup | `live/setup-tab.js` | reachable addresses, Wi-Fi change (reuses `steps/wifi.js`), GUEK status (masked, never fetched), firmware update and restart (`live/firmware-card.js`: "check for updates" against the release manifest and a confirmed install with download progress, only on the user's tap; file upload with header check and progress; confirmation by version and image identity; a two-step "restart device" that waits for a fresh uptime like the update does), MQTT broker and topic/payload templates with presets, a live preview rendered from `/api/live` by a port of the firmware's template code (`live/mqtt-template.js`) and template errors with their position (`live/mqtt-card.js`; the unsaved draft lives in `setup-tab.js` so closing the card keeps it; its status badge polls `/api/status` every 5 s while open), heap figures and 24 h heap trend (`live/mem-card.js`, polls `/api/heap` and `/api/status` every 5 min), event log (`live/log-card.js`), language, light/dark appearance (`theme.js`). All cards except connection and language collapse (`live/collapsible.js`): open/closed per browser, only the key starts open, a closed card shows a one-line summary and makes no requests; the key card is held open while the key is invalid, the firmware card during upload, download, reboot and restart. The firmware card is the one exception to "no requests while closed": it reads `/api/update` (a cached value, the device contacts no one) when the Setup tab opens, and resumes following an install already running (tab switch, or started from Home Assistant) |
...
rollback verdict without breaking a real device. `MOCK_UPDATE=0.7.0` makes "check for updates"
find 0.7.0 and the install come back running it (`MOCK_UPDATE=offline`: the check fails;
`MOCK_UPDATE_FAIL=1`: the download aborts halfway). Verified on mock 2026-09-25. The mock's `diag` uses an
...
|---|---|---|
| GET | `/api/status` | `{version, fw?, hostname, uptime, build, app, ota_auth, heap, mem:{free, min_free, largest, stack_loop, stack_httpd, stack_mqtt?}, hardware, meter:{preset, protocol, encrypted, key_hint?, …}, wifi:{…}, time:{valid, epoch}, history:{ok, addr, size, sectors, slots, interval, count, seq, oldest_qh, newest_qh, erases, writes, crc_errors}, mqtt?:{state, error, sent, dropped, skipped, last_ago}}`. `mqtt` (absent before 0.7.0) drives the MQTT card's badge: `state` `off`\|`connecting`\|`connected`\|`error`, `error` `tcp`\|`refused`\|`auth` for the connection or a `tpl_*` token when the templates stopped compiling after a profile change. `version` is the ESPHome release, `fw` the gPlug release (`gplug.yaml` `version`, absent before 0.6.0-rc.2); `app` is the first 16 hex digits of the running image's ELF SHA-256; the firmware card reads the same bytes at offset `0xB0` of the file it uploads and compares the two after the reboot, because `build` does not move when only embedded assets change. Both config objects are always present: `hardware` is `{}` until the hardware step is saved, `meter.protocol` is `0` (1 DSMR, 2 DLMS) until a profile is; `meter.key_hint` (4 hex digits of SHA-256 of the stored GUEK, only when one is stored, absent before 0.6.0) is shown under the meter step's "use stored key" toggle; the SPA opens the wizard unless `hardware.variant` and `meter.protocol` are set |
| GET | `/api/presets` | `{variants:[…], presets:[…]}` (see `../firmware/components/gplug_smi/presets.json`) |
| GET | `/api/wifi/scan` | `[{ssid, rssi, secure}]` |
| POST | `/api/key/check` | `{key}` → `{match}`. The meter step's "compare with the letter" check under the "use stored key" toggle; offered only when `meter.key_hint` is present (same firmware release). 400 `no stored key` / `key must be 32 hex chars`. Verified on gPlugK 2026-09-25 |
| POST | `/api/config/wifi` | `{ssid, psk}` |
| GET | `/api/config/mqtt` | `{enabled, host, port, client_id, user, password_set, mode, topic, payload, period, qos, retain, ctx:{device, mac}, fields:[{name, obis, unit, prec, string?}]}`. Never the password. `ctx` and `fields` feed the MQTT card's preview (`live/mqtt-template.js`), with values from `/api/live`. 404 before 0.7.0: the card is then not shown |
| POST | `/api/config/mqtt` | the GET fields minus `password_set`/`ctx`/`fields`, plus `password` only when the user typed one (left out = keep, `""` = clear). 400 `{error}` for a malformed body (`json`) or a field (`host`, `port`, `client_id`, `user`, `password`, `period`, `qos`, `mode`) or `{error: tpl_*, field: topic\|payload, pos, worst?}` for a template; the card maps both to text (`api.js` `DEVICE_ERRORS`). 503 `memory` when the device cannot allocate the ~8 kB to validate. Mock: `MOCK_MQTT=auth\|tcp` for a broker that refuses or cannot be reached |
...
| GET | `/api/history.csv?from=<qh>&to=<qh>` | Load-profile download (History tab → "export load profile"): every stored 15-min record, `text/csv` as attachment. `;`-separated, integers in Wh / W, local interval times; column names are the implementation's own (German). `from` inclusive, `to` exclusive quarter-hour indices (the tab derives them from a local date range); no params = everything including records without a timestamp. Navigated to, not fetched. Columns and the rules for empty cells: `../firmware/components/gplug_smi/history_csv.h`. Mock: `MOCK_HIST_DAYS=n` (default 40) |
...
| GET | `/api/heap` | `{period:300, uptime, samples:[[up_s, free_kb, min_kb, largest_kb],…]}`, oldest first, up to 288 (24 h, RAM only on the device). `live/mem-card.js` draws free heap and largest block. Mock: `MOCK_HEAP=leak npm run dev` for a falling trend plus a low-heap log entry |
...
| POST | `/api/reboot` | no body, `{ok:true}`, then the device logs `EV_RESTART` (code 11, detail 1) and restarts. The firmware card's restart polls `/api/status` until the uptime is younger than the wait (60 s timeout). Mock: API down 6 s, fresh uptime, and the restart plus its boot appear in `/api/log` |
| GET | `/api/update` | `{state, current, latest, newer, release_url, progress, checked_ago, error}`: the last release check, cached on the device -- reading it contacts no one. `state` is `unchecked`\|`checking`\|`none`\|`available`\|`installing`\|`error` (`unavailable` on a build without the update entity; 404 before 0.6.0-rc.2: the card then shows the file upload only). `newer` = `latest` is strictly newer than `current` by SemVer; an install is offered only then. `error` is `check` or `install`; `progress` 0–100 while installing; `checked_ago` s or `null`. Mock: `MOCK_UPDATE=0.7.0` offers that version, `MOCK_UPDATE=offline` fails the check, `MOCK_UPDATE_FAIL=1` aborts the download at 50 % |
| POST | `/api/update/check` | no body, `{ok:true}`; the device fetches the release manifest from github.io in the background (a no-op while one runs or within 10 s of the last). The card polls `/api/update` until `state` leaves `checking`. The only way the device ever contacts github.io: no check at boot or on a schedule |
| POST | `/api/update/install` | no body, `{ok:true}`, then the device downloads the image from the manifest, checks its MD5 and reboots. 409 `no update` unless `state` is `available` and `newer`; 401 `auth` (no `WWW-Authenticate`) when `ota_auth` and the Basic header (user `admin`) is missing or wrong. The card sends it only after a second, explicit confirmation, polls `/api/update` for `progress`, then waits for a fresh uptime and compares `fw` with `latest` |
